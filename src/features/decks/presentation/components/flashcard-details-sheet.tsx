...
export function FlashcardDetailsSheet({
  accentColor,
  audioSource,
  card,
  onClose,
  profile,
}: FlashcardDetailsSheetProps) {
  ...
  const progressWidth = toPercentage(progress);
            ...
            <ScrollView contentContainerStyle={styles.content}>
              ...
              <View style={styles.progressHeading}>
                <Text style={[styles.status, { color: reviewed ? accentColor : colors.textMuted }]}>
                  {reviewed ? String(profile?.reviewCount ?? 0) + " reviews" : "New"}
                </Text>
                <Text style={styles.caption}>
                  {reviewed
                    ? String(progress) + "% recall · " + explanation.historyBand
                    : "Not reviewed yet"}
                </Text>
              </View>
              <View style={styles.track}>
                <View
                  style={[styles.fill, { backgroundColor: accentColor, width: progressWidth }]}
                />
              </View>
              <View style={styles.ratings}>
                <Fact color={colors.danger} label="Again" value={profile?.againCount ?? 0} />
                <Fact color={colors.warning} label="Hard" value={profile?.hardCount ?? 0} />
                <Fact color={colors.success} label="Good" value={profile?.goodCount ?? 0} />
                <Fact color={colors.recallEasy} label="Easy" value={profile?.easyCount ?? 0} />
              </View>
              <Text style={styles.caption}>
...
type FactProps = Readonly<{ color: string; label: string; value: number }>;

function toPercentage(value: number): `${number}%` {
  return `${value}%`;
}
function Fact({ color, label, value }: FactProps) {
