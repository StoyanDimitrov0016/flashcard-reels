...
import {
  ...
  InMemoryReviewAttemptFinalizationTransaction,
...
describe("intra-session recurrence behavior", () => {
  it("Again schedules after the source using the configured distance and jitter", async () => {
    ...
    const recurrences = await harness.service.listSessionRecurrences(session.id);
    const recurrence = only(recurrences);
  ...
  it.each(["good", "easy"] as const)("%s does not retain a pending recurrence", async (rating) => {
    ...
    const recurrences = await harness.service.listSessionRecurrences(session.id);
    expect(recurrences.filter((recurrence) => recurrence.consumedAt === null)).toHaveLength(0);
  });
  ...
  it("moves colliding targets through every occupied position", async () => {
    ...
    const recurrences = await harness.service.listSessionRecurrences(session.id);
    expect(recurrences.map((recurrence) => recurrence.targetReelPosition)).toEqual([8, 9, 10]);
  });
  ...
  it("consumes a recurrence while retaining its history", async () => {
    ...
    const storedRecurrences = await harness.service.listSessionRecurrences(session.id);
    const stored = only(storedRecurrences);
    expect(stored.consumedAt).not.toBeNull();
    const remainingRecurrences = await harness.service.listSessionRecurrences(session.id);
    expect(
      remainingRecurrences.filter(
        (pendingRecurrence) =>
  ...
  it("finalizes attempts outside the five-reel editable window only in that session", async () => {
    const { harness, session } = await openMixedSession();
    const openedOtherSession = await harness.service.openSession(
      "focused",
      makeFlashcard(2).deckId,
      false
    );
    const otherSession = openedOtherSession.session;
    ...
    expect(requireValue(finalizedMixedAttempt).finalizedAt).not.toBeNull();
    expect(requireValue(finalizedFocusedAttempt).finalizedAt).toBeNull();
  });
  ...
  it("does not allow a finalized attempt to be rated again", async () => {
    ...
    const finalizedAttempt = await harness.attempts.findById(attemptId);
    expect(requireValue(finalizedAttempt).rating).toBeNull();
  });
  ...
  it("leaves an unrated presentation without a recall result", async () => {
    ...
    const unratedAttempt = await harness.attempts.findById(attemptId);
    expect(requireValue(unratedAttempt).rating).toBeNull();
    expect(await harness.service.listSessionRecurrences(session.id)).toEqual([]);
  });
  ...
  it("preserves pending recurrence state after service reconstruction", async () => {
    ...
    const reconstructed = new StudyServiceImpl(
      ...
      new InMemoryReviewAttemptFinalizationTransaction(harness.attempts),
      () => 0
    );
