...
Downloaded deck content and learner data have different lifetimes. The mobile SQLite schema keeps package-owned flashcards in `flashcards`, finalized rated reviews in `review_events`, current review summaries in `flashcard_progress`, and FSRS scheduler fields in `flashcard_memory_states`. The latter three store stable deck and flashcard IDs without cascading foreign keys to downloaded content. Unfinished `flashcard_review_attempts` remain session data; they are finalized and all progress aggregation for completed sessions is drained before a deck is removed.
